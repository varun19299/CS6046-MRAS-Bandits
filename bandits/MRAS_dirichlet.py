...
"""
import numpy as np
from scipy.stats import dirchlet
...
def MRAS_dirchlet(args, game_i, l=0.2, pbar=None, summation = 1000):
    ...
    for exp in range(args.repeat):
        ...
        for j in range(len(phases)):
            ...
            arms1 = np.random.dirchlet(dist,phases[j])
            ...
            for i in range(len(theta)):
                num = 0
                den = 0#np.zeros(len(game))
                for arm in arms1:
                    a = np.argmax(arm)
                    num+= np.exp(C*sample_mean[a])*(np.log(arms1[a][i]))/dirchlet.pdf(arm, theta)
                    den = den +np.exp(C*sample_mean[a])/dirchlet.pdf(arm, theta)
